...
const Chatbot: React.FC<ChatbotProps> = ({
  ...
  useEffect(() => {
    const loadExistingChat = async () => {
      ...
      console.log("isFromBookmarks", isFromBookmarks);
      if (isFromBookmarks) {
        debugger;
        console.log("bookrmarks", bookmarks);
  ...
  if (selectedBookmark.queries && Array.isArray(selectedBookmark.queries)) {
    for (const q of selectedBookmark.queries) {
      const response = await getQueryById(q.query_id); // You may want to use Promise.all for parallel requests
      const msgs = convertQueryResponseToChatMessages(response, selectedBookmark.id || selectedBookmark.bookmark_id);
